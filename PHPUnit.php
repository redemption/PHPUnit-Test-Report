...
define('PHPUnit_VERSION','3.5.5');
define('TEST_CASE_SUFFIX','Test.php');// default is 'TestCase.php', but we use Test.php to match with PHPUnit 3.5.x's new standards.

if (PHPUnit_VERSION >= 3.5) {
	require_once('PHPUnit/Autoload.php');
} else { //don't set include path
...
class PHPUnit
{
	...
	function __construct($testDir, $suite = '')
	{
		...
		foreach($objects as $name){
			if(strstr($name, TEST_CASE_SUFFIX) !== false) {
				$parts = explode('/', $name);
				$filename = end($parts);
				$filename = strtolower(str_replace(TEST_CASE_SUFFIX, '', $filename));
	...
	public function run($name = null) {
		...
		foreach(get_declared_classes() as $class) {
			if(preg_match("/Test$/",$class) && (strpos($class, 'PHPUnit_') === false)) {
				$testClasses[] = $class;
			}
		}
